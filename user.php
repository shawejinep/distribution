...
'account_deposit', 'account_log', 'account_detail', 'act_account', 'pay', 'default', 'bonus', 'group_buy', 'group_buy_detail', 'affiliate', 'comment_list','validate_email','track_packages', 'transform_points','qpassword_name', 'get_passwd_question', 'check_answer' , 'face','my_shop_name','my_up_shop_name','my_friend','my_brokerage','my_withdraw','friend_detail1','friend_detail2','friend_detail3');
/* 未登录处理 */
...
/* 
/* 分销中心 ：我的盟友*/
elseif ($action == 'my_friend')
{
	include_once(ROOT_PATH .'includes/lib_transaction.php');
	//查询总共多少等级
	$dengji =  count($affiliate['item']);
	$num_one = 0;
	$num_two = 0;
	$num_three = 0;
	//一级人数
	$sql_one = "select count(*) from ". $ecs->table('users') . "where parent_id=$user_id"; 
	$my_friend_one_num = $GLOBALS['db']->getOne($sql_one);
	$num_one = $my_friend_one_num['count(*)'];
    //二级人数
	$sql_two = "select user_id from ". $ecs->table('users') . "where parent_id=$user_id";
	$list_two_id = $GLOBALS['db']->getAll($sql_two);
	foreach ($list_two_id as $key =>$val)
	{
		$two_id = $val['user_id'];
		$sql_two_num = "select count(*) as two_num from ".$ecs->table('users') ." where parent_id=$two_id";
		
		$my_friend_two_count = $GLOBALS['db']->getOne($sql_two_num);
	    $num_two += $my_friend_two_count['two_num'];  
	}
	//三级人数
	foreach ($list_two_id as $key =>$val)
	{
		$two_id = $val['user_id'];
		$sql_two_num = "select user_id from ".$ecs->table('users') ." where parent_id=$two_id";
		$my_friend_two_id = $GLOBALS['db']->getAll($sql_two_num);	
		foreach ($my_friend_two_id as $k =>$v)
		{
			$three_id = $v['user_id'];
			$sql_three_num = "select count(*) as three_num from ".$ecs->table('users') ." where parent_id=$three_id";
			$my_friend_three_count = $GLOBALS['db']->getOne($sql_three_num);
			$num_three += $my_friend_three_count['three_num'];
		}
	}
    $smarty->assign('num_one',$num_one);
    $smarty->assign('num_two',$num_two);
    $smarty->assign('num_three',$num_three);
    
	$smarty->assign('my_friend', $my_friend);
	$smarty->display('user_transaction.dwt');
}


/* 分销中心 ： 我的盟友详情页*/
elseif ($action == 'friend_detail1')
{
	$smarty->assign('friend_detail1', $friend_detail1);
	//推荐注册分成
	$affiliate = unserialize($GLOBALS['_CFG']['affiliate']);
    ...
    for ($i = 1; $i<=$num; $i++)
    {
        ...
        if ($count)
        {
			...
			foreach($user_info as $key=>$value){
				...
				$sql="SELECT count(*) as order_num ,sum(fencheng)  as order_amount FROM ".$GLOBALS['ecs']->table('order_info')."WHERE user_id=".$value['user_id']." and order_status not in (2,4,7) " ;
...
//显示二级分销代理
elseif ($action == 'friend_detail2')
{
	$smarty->assign('friend_detail2', $friend_detail2);
	//推荐注册分成
	$affiliate = unserialize($GLOBALS['_CFG']['affiliate']);
	$user_list['user_list'] = array();
	$auid = $_SESSION['user_id'];
...
//显示三级分销代理
elseif ($action == 'friend_detail3')
{
	$smarty->assign('friend_detail3', $friend_detail3);
	//推荐注册分成
	$affiliate = unserialize($GLOBALS['_CFG']['affiliate']);
	$user_list['user_list'] = array();
	$auid = $_SESSION['user_id'];
	...
	for ($i = 1; $i<=$num; $i++)
	{
		...
		if ($count)
		{
			...
			foreach($user_info as $key=>$value){
				...
				/*甜  心 10 0修复 */
				$tianxin_level_money=$affiliate['item'][$k]['level_money'];
				$setmoney = round($order_info['order_amount'] * $tianxin_level_money, 2);
...
/* 分销中心 ：我的佣金*/
elseif ($action == 'my_brokerage')
{
	$smarty->assign('my_brokerage', $my_brokerage);
	include_once(ROOT_PATH .'includes/lib_transaction.php');
	$affiliate = unserialize($GLOBALS['_CFG']['affiliate']);
	$user_list['user_list'] = array();
	$auid = $_SESSION['user_id'];
	...
	for ($i = 1; $i<=$num; $i++)
	{
		...
		if ($count)
		{
			...
			$tianxinarr=array();
			foreach($user_info as $key=>$value){
				/*甜心100分销微分销开发未付款的佣金统计*/
				$sql="SELECT count(*) as order_num ,sum(fencheng)  as order_amount FROM ".$GLOBALS['ecs']->table('order_info')."WHERE user_id=".$value['user_id']." and  pay_status=0 and order_status not in (2,4,7)";
				$order_info=$db->getRow($sql);
				$k=$i-1;
				$affiliate['item'][$k]['level_money'] = (float)$affiliate['item'][$k]['level_money'];
				if($key==0){
				...
				$tianxinarr['weifukuan']['order_num']+=$order_info['order_num'];
				$tianxinarr['weifukuan']['order_amount']+=$order_info['order_amount'];
				$tianxinarr['weifukuan']['setmoney']+=$setmoney;
				/*甜心100分销微分销开发已经付款且未收货佣金统计*/
				...
				$tianxinarr['yifukuan']['order_num']+=$order_info['order_num'];
				$tianxinarr['yifukuan']['order_amount']+=$order_info['order_amount'];
				$tianxinarr['yifukuan']['setmoney']+=$setmoney;
	
				/*甜心100分销微分销开发已经收货佣金统计*/
				...
				$tianxinarr['yishouhuo']['order_num']+=$order_info['order_num'];
				$tianxinarr['yishouhuo']['order_amount']+=$order_info['order_amount'];
				$tianxinarr['yishouhuo']['setmoney']+=$setmoney;
			}
			foreach($tianxinarr as $key=>$vale){
				if($key=="weifukuan"){
	...
	//当前可提现余额
	$sql_my_surplus = "select user_money from " .$GLOBALS['ecs']->table('users')." WHERE user_id = '$user_id'";
	$surplus_amount = $GLOBALS['db']->getOne($sql_my_surplus);
	$smarty->assign('yi_surplus_amount',        $yi_surplus_amount);
	$smarty->assign('surplus_amount',        $surplus_amount);
	$smarty->assign('tianxin100arr',        $allarr);	
	$smarty->display('user_transaction.dwt');
}
