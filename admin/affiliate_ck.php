...
elseif ($_REQUEST['act'] == 'separate')
{
    ...
    $row = $db->getRow("SELECT o.order_sn, o.is_separate, o.fencheng, o.user_id, (o.goods_amount - o.discount) AS goods_amount FROM " . $GLOBALS['ecs']->table('order_info') . " o".
    ...
    if (empty($row['is_separate']))
    {
        ...
        if(empty($separate_by))
        {
            ...
            for ($i=0; $i < $num; $i++)
            {
                ...
                $row = $db->getRow("SELECT o.parent_id as user_id,u.user_name,o.user_name as personal_name,o.user_id as personal_id FROM " . $GLOBALS['ecs']->table('users') . " o" .
                ...
                {
                	/* 甜  心1 00 新增  会员等级对应的奖金 */
                	$award_scale = get_award_scale($up_uid);
					if(!empty($award_scale)){
						$setmoney=0;
					}
                	$setmoney += round($money * $award_scale*0.01, 2);
            ...
            if ($separate_personal > 0){
            	$personal_data = $db->getRow("SELECT o.user_id,u.user_name,u.rank_points FROM " . $GLOBALS['ecs']->table('order_info') . " o".
            			" LEFT JOIN " . $GLOBALS['ecs']->table('users') . " u ON o.user_id = u.user_id".
            			" WHERE order_id = '$oid'");
...
function write_affiliate_log($oid, $uid, $username, $money, $point, $separate_by,$separate_personal)
{
    $time = gmtime();
    $sql = "INSERT INTO " . $GLOBALS['ecs']->table('affiliate_log') . "( order_id, user_id, user_name, time, money, point, separate_type,separate_personal)".
                                                              " VALUES ( '$oid', '$uid', '$username', '$time', '$money', '$point', '$separate_by','$separate_personal')";
    if ($oid)
    {
